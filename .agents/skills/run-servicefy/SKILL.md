...
ServiceFY ITSM is a multi-tenant ITSM SaaS built with Vite + React 19 + TypeScript + Tailwind v4. Drive it via **`.Codex/skills/run-servicefy/driver.mjs`** against a running dev server — it uses Playwright's Node API directly (no `chromium-cli`).

Screenshots land in `.Codex/skills/run-servicefy/shots/`.
...
```bash
node .Codex/skills/run-servicefy/driver.mjs <command> [args]
```
...
node .Codex/skills/run-servicefy/driver.mjs screenshot
kill %1
```
