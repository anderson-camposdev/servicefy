...
test('monograma usa construção tipográfica profissional e separa S de FY', () => {
  ...
  assert.match(favicon, /data-variant="compact"/)
  assert.match(favicon, /data-segment="upper"/)
  assert.match(favicon, /data-segment="lower"/)
  assert.doesNotMatch(favicon, /<text/)
  assert.match(favicon, /stroke-linecap="round"/)
  assert.match(htmlShell, /href="\/favicon\.svg\?v=20260731"/)
})
