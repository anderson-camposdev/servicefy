-- Coluna de acumulador de reabertura (separada da pausa)
...
-- sla_events: permitir o novo tipo de evento
...
-- tg_handle_sla_pause(): novo ramo (C) de reabertura
...
BEGIN
  ...
  v_was_closed := OLD.state = ANY (v_closed_states);
  v_is_closed  := NEW.state = ANY (v_closed_states);
...
BEGIN
  ...
  IF OLD.state IS DISTINCT FROM NEW.state THEN
    ...
    ELSIF OLD.state = ANY (ARRAY['Resolved','Closed']) AND NOT (NEW.state = ANY (ARRAY['Resolved','Closed'])) THEN
      PERFORM public.sla_log_event(
        NEW.id, 'reopened',
        jsonb_build_object(
