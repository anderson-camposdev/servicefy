-- ServiceFY — corrige regressão introduzida pela migration 158.
--
-- A migration 158 passou a repassar mtta_minutes, mttr_minutes e
-- approval_paused_at de NEW para a tabela tickets dentro dos triggers
-- INSTEAD OF da view `incidents`. Só que a view `incidents` NUNCA expôs
-- essas 3 colunas no seu SELECT list (confirmado via pg_get_viewdef) —
-- então dentro do trigger, o record NEW simplesmente não tem esses
-- campos, e qualquer UPDATE feito através da view (inclusive
-- `UPDATE public.incidents SET is_response_breached = true ...` dentro
-- de check_sla_breaches/check_sla_warnings) passou a falhar com
-- "record "new" has no field "mtta_minutes"".
--
-- Efeito em produção: desde a aplicação da 158, os cron jobs
-- check_sla_breaches()/check_sla_warnings() (rodam a cada minuto) têm
-- check_sla_breaches falhando em toda execução — nenhum chamado é mais
-- verificado quanto a estouro de SLA, pior que o bug que a 158 tentava
-- corrigir (que ao menos notificava, ainda que repetidamente).
--
-- Correção: remove as 3 referências que a view não suporta. Se essas
-- colunas precisarem ser editáveis via a API no futuro, a coisa certa é
-- adicioná-las ao SELECT da view primeiro, não presumir que já existem.
...
BEGIN
  ...
  UPDATE public.tickets SET
    ...
    -- mtta_minutes, mttr_minutes e approval_paused_at REMOVIDOS: a view
    -- `incidents` não os expõe, então NEW nunca tem esses campos.
  WHERE id = OLD.id;
...
BEGIN
  INSERT INTO public.tickets (
    ...
    -- mtta_minutes, mttr_minutes e approval_paused_at REMOVIDOS (idem acima).
  ) VALUES (
