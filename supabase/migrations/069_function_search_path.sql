-- ============================================================
-- Flowfy ITSM — Migration 069
-- Fixar search_path das funções (hardening, sem mudança de comportamento).
--
-- Funções sem search_path explícito ficam sujeitas a "search_path
-- hijacking" (um schema malicioso no path pode sombrear objetos).
-- Fixamos `public, extensions, pg_temp` em todas as funções sinalizadas
-- pelo advisor de segurança do Supabase. Inclui as funções centrais
-- de autorização (is_current_user_msp_admin, get_current_user_*).
--
-- extensions entra no path para funções que usam pgcrypto (pg_trgm,
-- gen_random_uuid etc.); pg_temp por último (nunca antes de public).
-- ============================================================
