-- ============================================================
-- Flowfy ITSM — Migration 063
-- Flowfy BI v2 (3/4): RPC de CUBO genérico + drill-down.
--
-- bi_cube: agrega bi_tickets_unified por dimensões dinâmicas com
-- medidas dinâmicas, 100% no servidor. Segurança:
--   * SECURITY INVOKER — a RLS das tabelas base vale para o caller;
--   * guard explícito de tenant (padrão do projeto): não-MSP tem a
--     company forçada via get_current_user_company_id();
--   * NADA vindo do cliente vira identificador SQL: dimensões e
--     medidas são resolvidas pela whitelist (bi_dimensions /
--     bi_measures / bi_form_dimensions da migration 062);
--   * valores de filtro NUNCA são interpolados: as condições
--     referenciam o parâmetro JSONB ($) por índice.
--
-- bi_drilldown: lista paginada dos tickets por trás de um número
-- agregado (clicou no KPI -> vê os chamados).
-- ============================================================

-- ─── 1. Resolvedor de dimensão (whitelist) ───────────────────
-- Retorna a expressão SQL confiável para uma key de dimensão.
-- form.<label> -> (form_data->>'<label>'), com o label validado
-- contra bi_form_dimensions e citado como LITERAL (quote_literal).
...
-- ─── 2. Montador de condições de filtro ──────────────────────
-- Gera o SQL das condições referenciando o parâmetro JSONB de
-- filtros como $5 (bi_cube) ou $4 (bi_drilldown) por índice —
-- valores jamais interpolados. Ops: eq|neq|in|not_in|contains|
-- gte|lte|is_null|not_null.
CREATE OR REPLACE FUNCTION public.bi_build_filter_sql(
  p_filters     JSONB,
  p_company_id  UUID,
  p_param_ref   TEXT   -- ex: '$5'
) RETURNS TEXT
LANGUAGE plpgsql
STABLE
SECURITY INVOKER
...
BEGIN
  ...
  FOR v_i IN 0 .. v_n - 1 LOOP
    ...
    -- v_val referencia o valor DENTRO do parâmetro jsonb — nunca o literal.
...
DECLARE
  ...
  v_limit       INT;
BEGIN
  -- Guard de tenant (padrão do projeto)
  IF public.is_current_user_msp_admin() THEN
    v_company := p_company_id;  -- NULL = cross-tenant permitido para MSP
  ELSE
...
-- ────────────────────────────────────────────────────────────
-- Verificação (sanidade):
--   -- 1-dim deve bater com GROUP BY manual:
--   SELECT * FROM public.bi_cube(NULL, '{incident}', '{priority}', '{count}');
--   SELECT priority, count(*) FROM public.bi_tickets_unified
--    WHERE record_type='incident' AND created_at >= now()-interval '30 days' GROUP BY 1;
--   -- Soma das fatias = total sem dims:
--   SELECT * FROM public.bi_cube(NULL, '{incident}', '{}', '{count}');
--   -- Dimensão fora da whitelist deve falhar:
--   SELECT * FROM public.bi_cube(NULL, '{incident}', '{evil; DROP TABLE x}', '{count}');
--   -- Injection via form.* deve falhar na validação:
--   SELECT * FROM public.bi_cube(NULL, '{incident}', ARRAY['form.x'''); DROP TABLE incidents;--'], '{count}');
--   -- RLS: usuário de tenant A pedindo tenant B deve ver apenas A
--   (a company é forçada no guard e a view herda a RLS das bases).
-- ────────────────────────────────────────────────────────────
