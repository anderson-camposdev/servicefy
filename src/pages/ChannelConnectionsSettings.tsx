...
export default function ChannelConnectionsSettings({ companyId, activeRole, onBack }: {
  ...
  const revoke = async (id: string) => {
    if (!window.confirm('Desconectar este canal? O suporte deixará de receber ou enviar mensagens por ele até que seja reconfigurado.')) return
      ...
      <div className={`grid gap-5 ${showForm ? 'xl:grid-cols-[22rem_minmax(0,1fr)]' : ''}`}>
        {showForm && <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            {activeRole === 'sysadmin' && <label className="block text-xs font-bold">Quem usa esta conexão<select value={form.scope} onChange={event => setForm(v => ({ ...v, scope: event.target.value as 'tenant' | 'provider' }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="tenant">Somente esta empresa</option><option value="provider">Compartilhada entre várias empresas (parceiro de suporte)</option></select><span className="mt-1 block text-[11px] font-normal leading-4 text-slate-400">Escolha "Compartilhada" apenas se você administra o suporte de múltiplos clientes.</span></label>}
